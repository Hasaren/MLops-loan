...
#   학습 데이터와 인코드는 데이터분석가/데이터 기획자가 다루기 쉬운 컬럼명으로
#   만들어졌다. 두 상황을 잇는 번역 사전 역할
FIELD_TO_COLUMN = {
    "age": "나이",
    "gender": "성별",
    "annual_income": "연소득",
...
class LoanModel:
    '''
    대출 모델 아키텍쳐와 추론(예측) 규칙을 한 객체로 관리한다.
        파이프라인 뿐만 아니라 학습 시 사용한 범주형 인코더와 컬럼 순서도 함께 불러온다.
        운영 추론이 학습 전처리와 정확히 같아야 하는 MLOps의 핵심 원칙을 보여주는 구도.
    ...
    def __init__(self):
        ...
        # main.py에서 /heath (헬스체크)를 할 때 self.pipeline이 None이면 초기 상태를 표현
        self.pipeline = None
        self.label_encoders: dict[str, Any] = {}
        self.feature_names: list[str] = []

        self.threshold: float = 0.5 # 임계값
        # 확률을 가지고 승인/거절로 변환하는 기준 값
        # 금융권처럼 부실 대출(FP)의 비용이 큰 도메인에서는 이 값을 0.6~0.7로 올려
        #   더 보수적으로 심사하기도 한다.
    ...
    def load(self, model_dir: str = 'models') -> None:
        """
        모델, 레이블 인코더, 특성(컬럼) 순서 파일을 디스크에 불러온다.
        
        현재 작업폴더가 어디든 동일하게 동작하도록 이 파일의 위치를 기준으로 계산
        Dockerfile의 WORKDIR이 달라지든 이 코드는 항상 이 파일 기준으로 models폴더를
        찾으므로 실행 위치에 영향을 받지 않는다.
        """
        script_dir = Path(__file__).parent # app폴더 (현재 파일이 있는 폴더)
        model_path = script_dir.parent / model_dir # MLops/models
        ...
        if not feature_names_path.exists():
            raise FileNotFoundError(f'특징 컬럼 이름 파일을 찾을 수 없습니다: {feature_names_path}')
